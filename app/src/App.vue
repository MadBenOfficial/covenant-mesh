...
const hasOrganization=computed(()=>ownedOrganizations.value.length>0);
...
async function load(){loading.value=true;dataError.value="";try{const v=[];for(const name of ["get_overview","get_organizations","get_collections","get_requests","get_permits","get_usage","get_audits","get_remediations"])v.push(await readContract(name));[overview.value,organizations.value,collections.value,requests.value,permits.value,usage.value,audits.value,remediations.value]=v;if(!selectedCollection.value&&collections.value.length)selectedCollection.value=collections.value[0].id;const mine=organizations.value.find(x=>sameAddress(x.owner,wallet.value));if(mine&&!ownedOrganizations.value.some(x=>x.id===requestForm.value.organization_id))requestForm.value.organization_id=mine.id}catch(e){dataError.value=formatError(e)}finally{loading.value=false}}
async function transact(title,functionName,args){tx.value={open:true,stage:"signature",title,hash:"",error:""};try{const r=await writeContract({client:client.value,functionName,args,onStage:(stage,hash)=>{tx.value.stage=stage;if(hash)tx.value.hash=hash}});tx.value.hash=r.hash;action.value=null;await load()}catch(e){tx.value.stage="failed";tx.value.error=formatError(e)}}
const submitRequest=()=>{if(!requestForm.value.organization_id){action.value="createOrg";return}return transact("Submit access request","submit_access_request",[requestForm.value.organization_id,currentCollection.value.id,requestForm.value.title,requestForm.value.purpose,requestForm.value.methods,requestForm.value.safeguards,Number(requestForm.value.retention_days),Number(requestForm.value.requested_budget)])};
  ...
  <div v-else class="workspace">
    <header class="app-head">
      ...
      <div class="head-actions">
        <select v-if="ownedOrganizations.length" v-model="requestForm.organization_id" class="org-select" title="Active organization"><option v-for="org in ownedOrganizations" :key="org.id" :value="org.id">{{org.name}}</option></select>
    ...
    <div v-if="action" class="modal-backdrop" @click.self="action=null">
      <section v-if="action==='createOrg'" class="modal compact">
        ...
        <label>Organization name<input v-model="createOrgForm.name" placeholder="Civic Systems Lab"></label><label>Organization type<select v-model="createOrgForm.kind"><option value="RESEARCH">Research</option><option value="AUDITOR">Auditor</option><option value="STEWARD">Steward</option></select></label>
